...
titi <- titi %>% filter(Plot %in% 1:15) %>%  arrange(id,time)
# ladders <- titi %>%
  # filter(id %in% ids)
...
singlets <- titi %>% filter(id%in%id_singlets & time==2017 & CodeAlive) %>% select(id, Plot) %>% unique
singlets %>% group_by(Plot) %>% summarise(count = n())

duets <- titi %>% filter(id%in%id_duets & time==2017 & CodeAlive) %>% select(id, Plot) %>% unique
duets %>% group_by(Plot) %>% summarise(count = n())

triplets <- titi %>% filter(id%in%id_triplets & time==2017 & CodeAlive) %>% select(id, Plot) %>% unique
...
trees_tocheck <- singlets %>%
  ...
  left_join(titi %>%
              filter(time == 2017) %>%
              select(id, Xutm,Yutm,Xfield,Yfield,Genus,Species, POM,measure) %>%
